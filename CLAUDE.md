...
- `ko mcp inspect <url>` / `ko mcp call <url> <tool>` — investigate/use an MCP server (ko is itself an MCP client). Verb takes a `<url>` = client; a future `ko mcp serve` (no url) = ko as server. `inspect` lists tools + resources + prompts (+ capabilities + the server's instructions); `--tool <name>` dumps that tool's full JSON input schema. `call` invokes a tool (`--arg k=v`). `-H 'Authorization: Bearer ...'` for auth, `--json` everywhere. On failure prints the server's real HTTP status + body (e.g. a 503 "MCP endpoint is not configured" → server-side issue, not your client). Listings to stdout, banner to stderr.
