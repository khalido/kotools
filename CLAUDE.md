...
- `ko logs [--n] [--errors] [--json]` — recent local command logs. ko writes **one structured JSON event per command** (`~/.local/state/ko/logs/<date>.jsonl`: cmd label, duration, exit code, error type — **no args/secrets**, following PostHog's "one wide event per request"). On by default; `[logs] enabled = false` (or `[logs] level = "debug"`) in config.toml. Local-only; a PostHog sync is opt-in/later. Logging is best-effort — never breaks a command.
