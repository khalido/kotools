...
- [ ] **Inject the current date into agent instructions** (noted 2026-07-14, surfaced when tv gained memory). Agents don't know today's date, so any self-dated memory entry is a hallucinated guess (the tv smoke test wrote "2024-05-22" in 2026). Thin fix: a shared `@agent.instructions` (or a line in `instructions_block`) adding `Today is {date}.` — one place, helps every memory-carrying agent date entries correctly and reason about recency (paper/HN "last N months", "is this stale").
