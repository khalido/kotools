...
- [ ] **`ko llm "<prompt>"`** — one-shot LLM call, stdin-aware: `ko hn item 123 | ko llm "summarize the debate"`. No tools, no agent loop — pydantic-ai `Agent.run_sync` with stdin appended as context, `-m` for model (cheap default). The collate step every skill recipe ends with; the glue that makes plain CLI pipelines agentic. Supersedes the old `ko summarise` idea (a summarize is just one prompt to `ko llm`).
