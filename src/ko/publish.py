...
# --- llms.txt: agent-facing site index, generated from the md pages ---
# llmstxt.org shape (H1 + `>` summary + a `## Pages` link list), with one opinionated
# extension: sites of ≤ _LLMS_EXCERPT_MAX_PAGES pages inline their (truncated) content,
# so a tiny site is fully readable in the ONE llms.txt fetch.

LLMS_MARKER = (
    "<!-- generated by ko publish — delete this line to hand-maintain this file -->"
)
_LLMS_DESC_CHARS = 200
_LLMS_EXCERPT_CHARS = 1500
...
def _page_info(path: Path) -> dict:
    """title/desc/tags/body for one md page. Front matter wins (`title:`,
    `description:`/`summary:`/`desc:`, `tags:`); fallbacks are the first `# ` heading
    and the first prose paragraph."""
    try:
    ...
    desc = meta.get("description") or meta.get("summary") or meta.get("desc")
    if (
        not desc
    ):  # first prose line — skip headings, HTML, fences, tables, quotes, lists
        desc = next(
            (
                ln
                for ln in lines
    ...
    if len(desc) > _LLMS_DESC_CHARS:
        desc = desc[:_LLMS_DESC_CHARS].rsplit(" ", 1)[0] + "…"
    tags = meta.get("tags", "").strip("[]")
    return {"title": title, "desc": desc, "tags": tags, "body": body}


def content_dir(folder: Path) -> Path:
    """Where a site's md content lives: `public/` on a --hono worker, else the folder."""
    pub = folder / "public"
    return pub if (pub / "index.html").exists() else folder


def is_md_site(folder: Path) -> bool:
    """Is this a ko markdown-shell site (the only kind llms.txt can be generated for)?
    Detected by README.md + the md-rendering shell, not by how it was scaffolded."""
    d = content_dir(folder)
    try:
        return (d / "README.md").exists() and "markdownit" in (
            d / "index.html"
...
def generate_llms_txt(folder: Path) -> str | None:
    """Build llms.txt from a site's top-level md pages (README first — its title/first
    paragraph become the H1/summary). None when `folder` isn't an md site."""
    if not is_md_site(folder):
        return None
    d = content_dir(folder)
    pages = sorted(d.glob("*.md"), key=lambda p: (p.name != "README.md", p.name))
    infos = [(p, _page_info(p)) for p in pages if p.name != "CLAUDE.md"]
    infos = [(p, i) for p, i in infos if i]
    if not infos:
        return None
    lines = [f"# {infos[0][1]['title']}", ""]
    if infos[0][1]["desc"]:
        lines += [f"> {infos[0][1]['desc']}", ""]
    lines += [
        "Every page here is also served as raw markdown at its `.md` path.",
        "",
        "## Pages",
        "",
    ]
    for p, i in infos:
        note = i["desc"] if p.name != "README.md" else "the homepage / hub"
        if i["tags"]:
            note = f"{note} ({i['tags']})" if note else f"({i['tags']})"
        lines.append(
            f"- [{i['title']}](/{p.name}): {note}"
            if note
            else f"- [{i['title']}](/{p.name})"
        )
    if len(infos) <= _LLMS_EXCERPT_MAX_PAGES:  # tiny site → whole thing in one fetch
        for p, i in infos:
            body = re.sub(r"^# .*\n+", "", i["body"].strip(), count=1)
            if len(body) > _LLMS_EXCERPT_CHARS:
                body = (
                    body[:_LLMS_EXCERPT_CHARS].rsplit("\n", 1)[0]
                    + f"\n\n[…truncated — full text at /{p.name}]"
                )
            lines += ["", f"## {i['title']} (/{p.name})", "", body]
    lines += ["", LLMS_MARKER, ""]
    return "\n".join(lines)
...
def deploy(folder: Path, name: str | None = None, force: bool = False) -> str:
    ...
    write_llms_txt(folder)  # md sites: refresh the agent-facing index from the pages
...
def scaffold(
    ...
    if mode in ("md", "hono") and (seeded := write_llms_txt(folder)):
        written.append(seeded)  # agent-facing index; regenerated on every publish
    return written
  ...
  <body>
    ...
    <script type="module">
      ...
      if (!res || !res.ok) {
        ...
        // front matter (llms.txt metadata: title/description/tags) is data, not prose
        if (src.startsWith("---\\n")) {
...
- **`llms.txt` is generated on every publish** — an agent-facing index of the md pages
  (llmstxt.org shape; ≤2-page sites inline their truncated content). Per page it uses front
  matter `title:` / `description:` (or `summary:`) / `tags:` when present, else the first `# `
  heading + first paragraph — front matter is stripped before rendering, so it's safe to add.
  Don't edit `llms.txt` by hand unless you delete its marker line (that opts the file out of
  regeneration for good).
...
- `public/` — the static site (markdown doc by default: edit `public/README.md`, the nav hub;
  ...
  `public/llms.txt` is **generated on every publish** from the md pages (front matter
  `title:`/`description:`/`tags:` win, else H1 + first paragraph) — same rules as the `--md`
  scaffold; delete its marker line to hand-maintain it. Note: on a PIN-gated site the gate
  covers `/llms.txt` too (agents need the cookie), which may be exactly what you want.
